...
static int tty_keys_extended_key(struct tty *, const char *, size_t, size_t *,
                                 key_code *);
...
static const struct tty_default_key_xterm tty_default_xterm_keys[] = {
    {"\033[1;_P", KEYC_F1},    {"\033O1;_P", KEYC_F1},
    {"\033O_P", KEYC_F1},      {"\033[1;_Q", KEYC_F2},
    {"\033O1;_Q", KEYC_F2},    {"\033O_Q", KEYC_F2},
    {"\033[1;_R", KEYC_F3},    {"\033O1;_R", KEYC_F3},
    {"\033O_R", KEYC_F3},      {"\033[1;_S", KEYC_F4},
    {"\033O1;_S", KEYC_F4},    {"\033O_S", KEYC_F4},
    {"\033[15;_~", KEYC_F5},   {"\033[17;_~", KEYC_F6},
    {"\033[18;_~", KEYC_F7},   {"\033[19;_~", KEYC_F8},
    {"\033[20;_~", KEYC_F9},   {"\033[21;_~", KEYC_F10},
    {"\033[23;_~", KEYC_F11},  {"\033[24;_~", KEYC_F12},
    {"\033[1;_A", KEYC_UP},    {"\033[1;_B", KEYC_DOWN},
    {"\033[1;_C", KEYC_RIGHT}, {"\033[1;_D", KEYC_LEFT},
    {"\033[1;_H", KEYC_HOME},  {"\033[1;_F", KEYC_END},
    {"\033[5;_~", KEYC_PPAGE}, {"\033[6;_~", KEYC_NPAGE},
    {"\033[2;_~", KEYC_IC},    {"\033[3;_~", KEYC_DC},
};
...
void tty_keys_free(struct tty *tty) { tty_keys_free1(tty->key_tree); }
...
static struct tty_key *tty_keys_find(struct tty *tty, const char *buf,
...
/* Find the next node. */
static struct tty_key *tty_keys_find1(struct tty_key *tk, const char *buf,
                                      size_t len, size_t *size) {
  /* If no data, no match. */
  if (len == 0) {
    return NULL;
...
static int tty_keys_next1(struct tty *tty, const char *buf, size_t len,
                          key_code *key, size_t *size, int expired) {
  ...
  log_debug("%s: next key is %zu (%.*s) (expired=%d)", c->name, len, (int)len,
            buf, expired);
  ...
  if (more == UTF8_MORE) {
    ...
    *key = uc;
    ...
    log_debug("%s: UTF-8 key %.*s %#llx", c->name, (int)ud.size, ud.data, *key);
    return 0;
  }
...
first_key:
  ...
  if (onlykey < 0x20 && onlykey != C0_HT && onlykey != C0_CR &&
      onlykey != C0_ESC) {
    onlykey |= 0x40;
...
/* Key timer callback. */
static void tty_keys_callback(__unused int fd, __unused short events,
                              void *data) {
  struct tty *tty = data;
 ...
 */
static int tty_keys_extended_key(struct tty *tty, const char *buf, size_t len,
  ...
  *size = end + 1;
  ...
  if (log_get_level() != 0) {
    log_debug("%s: extended key %.*s is %llx (%s)", c->name, (int)*size, buf,
              nkey, key_string_lookup_key(nkey, 1));
  }
 ...
 * sequence.
 */
static int tty_keys_mouse(struct tty *tty, const char *buf, size_t len,
  ...
  *size = 0;
  ...
  if (buf[2] == 'M') {
    ...
    *size = 3;
    ...
    if (b < MOUSE_PARAM_BTN_OFF || x < MOUSE_PARAM_POS_OFF ||
 ...
 * partial.
 */
static int tty_keys_clipboard(struct tty *tty, const char *buf, size_t len,
                              size_t *size) {
  struct client *c = tty->client;
 ...
 */
static int tty_keys_device_attributes(struct tty *tty, const char *buf,
                                      size_t len, size_t *size) {
  struct client *c = tty->client;
 ...
 * failure, 1 for partial.
 */
static int tty_keys_device_attributes2(struct tty *tty, const char *buf,
                                       size_t len, size_t *size) {
  struct client *c = tty->client;
  int *features = &c->term_features;
  u_int i, n = 0;
 ...
 * failure, 1 for partial.
 */
static int tty_keys_extended_device_attributes(struct tty *tty, const char *buf,
                                               size_t len, size_t *size) {
  struct client *c = tty->client;
  int *features = &c->term_features;
  u_int i;
 ...
 * failure, 1 for partial.
 */
int tty_keys_colours(struct tty *tty, const char *buf, size_t len, size_t *size,
                     int *fg, int *bg) {
  struct client *c = tty->client;
  u_int i;
  char tmp[128];
