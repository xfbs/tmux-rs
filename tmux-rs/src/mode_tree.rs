use compat_rs::{
    impl_tailq_entry,
    queue::{tailq_concat, tailq_empty, tailq_init, tailq_insert_tail, tailq_last, tailq_prev, tailq_remove},
    strlcat,
};
...
unsafe extern "C" {
    ...
    // pub fn mode_tree_start( _: *mut window_pane, _: *mut args, _: mode_tree_build_cb, _: mode_tree_draw_cb, _: mode_tree_search_cb, _: mode_tree_menu_cb, _: mode_tree_height_cb, _: mode_tree_key_cb, _: *mut c_void, _: *const menu_item, _: *mut *const c_char, _: c_uint, _: *mut *mut screen,) -> *mut mode_tree_data;
...
#[unsafe(no_mangle)]
unsafe extern "C" fn mode_tree_find_item(mtl: *mut mode_tree_list, tag: u64) -> *mut mode_tree_item {
    unsafe {
...
#[unsafe(no_mangle)]
unsafe extern "C" fn mode_tree_build_lines(mtd: *mut mode_tree_data, mtl: *mut mode_tree_list, depth: u32) {
    unsafe {
        ...
        (*mtd).depth = depth;
        for mti in tailq_foreach(mtl).map(NonNull::as_ptr) {
            (*mtd).line_list = xreallocarray_((*mtd).line_list, (*mtd).line_size as usize + 1).as_ptr();
            ...
            if let Some(keycb) = (*mtd).keycb {
                (*mti).key = keycb(NonNull::new((*mtd).modedata).unwrap(), NonNull::new((*mti).itemdata).unwrap(), (*mti).line);
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn mode_tree_get_current(mtd: *mut mode_tree_data) -> NonNull<c_void> { NonNull::new(unsafe { (*(*(*mtd).line_list.add((*mtd).current as usize)).item).itemdata }).unwrap() }

#[unsafe(no_mangle)]
pub unsafe extern "C" fn mode_tree_get_current_name(mtd: *mut mode_tree_data) -> *const c_char { unsafe { (*(*(*mtd).line_list.add((*mtd).current as usize)).item).name } }
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn mode_tree_get_tag(mtd: *mut mode_tree_data, tag: u64, found: *mut u32) -> i32 {
    unsafe {
        let mut i = 0;
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn mode_tree_each_tagged(mtd: *mut mode_tree_data, cb: mode_tree_each_cb, c: *mut client, key: key_code, current: i32) {
    unsafe {
        let mut fired = false;
        for i in 0..(*mtd).line_size {
            let mti = (*(*mtd).line_list.add(i as usize)).item;
            if (*mti).tagged != 0 {
                fired = true;
                cb.unwrap()(NonNull::new((*mtd).modedata).unwrap(), NonNull::new((*mti).itemdata).unwrap(), c, key);
            }
        }
        if !fired && current != 0 {
...
pub unsafe extern "C" fn mode_tree_build(mtd: *mut mode_tree_data) {
    unsafe {
        ...
        let mut tag = if !(*mtd).line_list.is_null() { (*(*(*mtd).line_list.add((*mtd).current as usize)).item).tag } else { u64::MAX };

        tailq_concat(&raw mut (*mtd).saved, &raw mut (*mtd).children);
        tailq_init(&raw mut (*mtd).children);

        (*mtd).buildcb.unwrap()(NonNull::new((*mtd).modedata).unwrap(), &raw mut (*mtd).sort_crit, &raw mut tag, (*mtd).filter);
        (*mtd).no_matches = tailq_empty(&raw mut (*mtd).children) as i32;
        if (*mtd).no_matches != 0 {
            (*mtd).buildcb.unwrap()(NonNull::new((*mtd).modedata).unwrap(), &raw mut (*mtd).sort_crit, &raw mut tag, null_mut());
        }
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn mode_tree_add(mtd: *mut mode_tree_data, parent: *mut mode_tree_item, itemdata: *mut c_void, tag: u64, name: *const c_char, text: *const c_char, expanded: i32) -> *mut mode_tree_item {
    unsafe {
    ...
    unsafe {
        ...
        'done: {
            ...
            for i in 0..(*mtd).line_size {
                ...
                if ((*mti).key != KEYC_NONE) {
                    xasprintf(&raw mut key, c"(%s)%*s".as_ptr(), (*mti).keystr, pad, c"".as_ptr());
                } else {
                ...
                if ((*line).depth == 0) {
                    ...
                    for j in 1..(*line).depth {
                        if !(*mti).parent.is_null() && (*(*mtd).line_list.add((*(*mti).parent).line as usize)).last != 0 {
                ...
                let tag = if (*mti).tagged != 0 { c"*".as_ptr() } else { c"".as_ptr() };
                xasprintf(&raw mut text, c"%-*s%s%s%s%s".as_ptr(), keylen, key, start, (*mti).name, tag, if !(*mti).text.is_null() { c": ".as_ptr() } else { c"".as_ptr() });
                ...
                if (i != (*mtd).current) {
                    ...
                    screen_write_nputs(&raw mut ctx, w as isize, &raw mut gc0, c"%s".as_ptr(), text);
                    if !(*mti).text.is_null() {
                        format_draw(&raw mut ctx, &raw mut gc0, w - width, (*mti).text, null_mut(), 0);
                    }
                } else {
                    screen_write_clearendofline(&raw mut ctx, gc.bg as u32);
                    screen_write_nputs(&raw mut ctx, w as isize, &raw mut gc, c"%s".as_ptr(), text);
                    if !(*mti).text.is_null() {
                        format_draw(&raw mut ctx, &raw mut gc, w - width, (*mti).text, null_mut(), 0);
                    }
                }
                free_(text);
                free_(key);
            ...
            screen_write_box(&raw mut ctx, w, sy - h, box_lines::BOX_LINES_DEFAULT, null(), null());
            ...
            if !(*mtd).sort_list.is_null() {
                xasprintf(
                    ...
                    *(*mtd).sort_list.add((*mtd).sort_crit.field as usize),
                    if (*mtd).sort_crit.reversed != 0 { c", reversed".as_ptr() } else { c"".as_ptr() },
                );
            } else {
            ...
            if w - 2 >= strlen(text) as u32 {
                ...
                let n = if (*mtd).no_matches != 0 { "no matches".len() } else { "active".len() };
            ...
            if (box_x != 0 && box_y != 0) {
                ...
                (*mtd).drawcb.unwrap()((*mtd).modedata, NonNull::new((*mti).itemdata), &raw mut ctx, box_x, box_y);
            }
        }
        // done:
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn mode_tree_search_backward(mtd: *mut mode_tree_data) -> *mut mode_tree_item {
    unsafe {
        ...
        loop {
            ...
            if searchcb((*mtd).modedata, NonNull::new((*mti).itemdata).unwrap(), (*mtd).search).as_bool() {
                return mti;
            }
        }
        null_mut()
...
pub unsafe extern "C" fn mode_tree_search_set(mtd: *mut mode_tree_data) {
    unsafe {
        let mti = if (*mtd).search_dir == mode_tree_search_dir::MODE_TREE_SEARCH_FORWARD { mode_tree_search_forward(mtd) } else { mode_tree_search_backward(mtd) };
        if mti.is_null() {
            return;
        }
        let tag = (*mti).tag;
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn mode_tree_search_callback(_c: *mut client, data: NonNull<c_void>, s: *const c_char, _done: i32) -> i32 {
    unsafe {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn mode_tree_filter_callback(_c: *mut client, data: NonNull<c_void>, s: *const c_char, _done: i32) -> i32 {
    unsafe {
        let mut mtd: *mut mode_tree_data = data.cast().as_ptr();
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn mode_tree_menu_callback(_menu: *mut menu, _idx: u32, key: key_code, data: *mut c_void) {
    unsafe {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn mode_tree_display_menu(mtd: *mut mode_tree_data, c: *mut client, mut x: u32, y: u32, outside: i32) {
    unsafe {
        // struct mode_tree_item *mti;
        // struct menu *menu;
        ...
        let mut line = if ((*mtd).offset + y > (*mtd).line_size - 1) { (*mtd).current } else { (*mtd).offset + y };
        ...
        if menu_display(menu, 0, 0, null_mut(), x, y, c, box_lines::BOX_LINES_DEFAULT, null_mut(), null_mut(), null_mut(), null_mut(), Some(mode_tree_menu_callback), mtm.cast()) != 0 {
            menu_free(menu);
        }
    }
}
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn mode_tree_key(mtd: *mut mode_tree_data, c: *mut client, key: *mut key_code, m: *mut mouse_event, xp: *mut u32, yp: *mut u32) -> i32 {
    unsafe {
        // mode_tree_line *line;
        ...
        if KEYC_IS_MOUSE(*key) && !m.is_null() {
            ...
            if ((*mtd).offset + y < (*mtd).line_size) {
                if (*key == keyc::KEYC_MOUSEDOWN1_PANE as u64 || *key == keyc::KEYC_MOUSEDOWN3_PANE as u64 || *key == keyc::KEYC_DOUBLECLICK1_PANE as u64) {
        ...
        match *key {
            ...
            code::T_CTRL => {
                for i in 0..(*mtd).line_size {
                    if ((*(*(*mtd).line_list.add(i as usize)).item).parent.is_null() && (*(*(*mtd).line_list.add(i as usize)).item).no_tag == 0) || (!(*(*(*mtd).line_list.add(i as usize)).item).parent.is_null() && (*(*(*(*mtd).line_list.add(i as usize)).item).parent).no_tag != 0) {
            ...
            code::QUESTION_MARK | code::SLASH | code::S_CTRL => {
                (*mtd).references += 1;
                status_prompt_set(c, null_mut(), c"(search) ".as_ptr(), c"".as_ptr(), Some(mode_tree_search_callback), Some(mode_tree_search_free), mtd.cast(), PROMPT_NOFORMAT, prompt_type::PROMPT_TYPE_SEARCH);
            }
            code::N => {
            ...
            code::F => {
                (*mtd).references += 1;
                status_prompt_set(c, null_mut(), c"(filter) ".as_ptr(), (*mtd).filter, Some(mode_tree_filter_callback), Some(mode_tree_filter_free), mtd.cast(), PROMPT_NOFORMAT, prompt_type::PROMPT_TYPE_SEARCH);
            }
            code::V => {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn mode_tree_run_command(c: *mut client, fs: *mut cmd_find_state, template: *const c_char, name: *const c_char) {
    unsafe {
        let mut error: *mut c_char = null_mut();

        let command = cmd_template_replace(template, name, 1);
        if !command.is_null() && *command != b'\0' as i8 {
            let state = cmdq_new_state(fs, null_mut(), 0);
