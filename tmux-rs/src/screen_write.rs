use compat_rs::{
    ...
    queue::{tailq_head_initializer, tailq_remove},
};
...
unsafe extern "C" {
    fn screen_write_collect_trim(_: *const screen_write_ctx, _: u32, _: u32, _: u32, _: *mut i32) -> *mut screen_write_citem;
    fn screen_write_collect_clear(_: *const screen_write_ctx, _: u32, _: u32);
    fn screen_write_collect_scroll(_: *const screen_write_ctx, _: u32);
    fn screen_write_collect_flush(_: *const screen_write_ctx, _: i32, _: *const c_char);
    fn screen_write_overwrite(_: *const screen_write_ctx, _: *mut grid_cell, _: u32) -> i32;
    fn screen_write_combine(_: *const screen_write_ctx, _: *const grid_cell) -> i32;
}
...
#[unsafe(no_mangle)]
unsafe extern "C" fn
static void screen_write_offset_timer(__unused int fd, __unused short events,
                                      void *data) {
...
#[unsafe(no_mangle)]
unsafe extern "C" fn
static void screen_write_set_cursor(struct screen_write_ctx *ctx, int cx,
                                    int cy) {
...
#[unsafe(no_mangle)]
unsafe extern "C" fn
static int screen_write_set_client_cb(struct tty_ctx *ttyctx,
                                      struct client *c) {
  struct window_pane *wp = (*ttyctx).arg;
...
#[unsafe(no_mangle)]
unsafe extern "C" fn
static void screen_write_initctx(struct screen_write_ctx *ctx,
                                 struct tty_ctx *ttyctx, int sync) {
  struct screen *s = (*ctx).s;
...
#[unsafe(no_mangle)]
unsafe extern "C" fn
void screen_write_start_pane(struct screen_write_ctx *ctx,
  ...
  if (log_get_level() != 0) {
    log_debug("%s: size %ux%u, pane %%%u (at %u,%u)", __func__,
              screen_size_x((*ctx).s), screen_size_y((*ctx).s), (*wp).id,
              (*wp).xoff, (*wp).yoff);
  }
}
...
#[unsafe(no_mangle)]
unsafe extern "C" fn
void screen_write_start_callback(struct screen_write_ctx *ctx, struct screen *s,
...
void screen_write_start(struct screen_write_ctx *ctx, struct screen *s) {
  ...
  if (log_get_level() != 0) {
    log_debug("%s: size %ux%u, no pane", __func__, screen_size_x((*ctx).s),
              screen_size_y((*ctx).s));
  }
}
...
#[unsafe(no_mangle)]
unsafe extern "C" fn
void screen_write_putc(struct screen_write_ctx *ctx,
                       const struct grid_cell *gcp, u_char ch) {
  struct grid_cell gc;
...
#[unsafe(no_mangle)]
unsafe extern "C" fn
int screen_write_text(struct screen_write_ctx *ctx, u_int cx, u_int width,
                      u_int lines, int more, const struct grid_cell *gcp,
                      const char *fmt, ...) {
  struct screen *s = (*ctx).s;
  va_list ap;
  char *tmp;
...
void screen_write_nputs(struct screen_write_ctx *ctx, ssize_t maxlen,
                        const struct grid_cell *gcp, const char *fmt, ...) {
  va_list ap;
...
#[unsafe(no_mangle)]
unsafe extern "C" fn
void screen_write_vnputs(struct screen_write_ctx *ctx, ssize_t maxlen,
                         const struct grid_cell *gcp, const char *fmt,
                         va_list ap) {
  struct grid_cell gc;
...
#[unsafe(no_mangle)]
unsafe extern "C" fn
void screen_write_fast_copy(struct screen_write_ctx *ctx, struct screen *src,
...
#[unsafe(no_mangle)]
unsafe extern "C" fn
static void screen_write_box_border_set(enum box_lines lines, int cell_type,
                                        struct grid_cell *gc) {
  switch (lines) {
  case BOX_LINES_NONE:
    break;
...
#[unsafe(no_mangle)]
unsafe extern "C" fn
void screen_write_hline(struct screen_write_ctx *ctx, u_int nx, int left,
                        int right, enum box_lines lines,
...
#[unsafe(no_mangle)]
unsafe extern "C" fn
void screen_write_vline(struct screen_write_ctx *ctx, u_int ny, int top,
                        int bottom) {
  struct screen *s = (*ctx).s;
  struct grid_cell gc;
  u_int cx, cy, i;
...
#[unsafe(no_mangle)]
unsafe extern "C" fn
void screen_write_menu(struct screen_write_ctx *ctx, struct menu *menu,
                       int choice, enum box_lines lines,
  ...
  screen_write_box(ctx, (*menu).width + 4, (*menu).count + 2, lines, border_gc,
                   (*menu).title);
...
void screen_write_box(struct screen_write_ctx *ctx, u_int nx, u_int ny,
                      enum box_lines lines, const struct grid_cell *gcp,
                      const char *title) {
  struct screen *s = (*ctx).s;
  struct grid_cell gc;
  u_int cx, cy, i;
...
#[unsafe(no_mangle)]
unsafe extern "C" fn
void screen_write_preview(struct screen_write_ctx *ctx, struct screen *src,
                          u_int nx, u_int ny) {
  struct screen *s = (*ctx).s;
  struct grid_cell gc;
  u_int cx, cy, px, py;
  ...
  if ((*src).mode & MODE_CURSOR) {
    ...
    screen_write_set_cursor(ctx, cx + ((*src).cx - px), cy + ((*src).cy - py));
    screen_write_cell(ctx, &gc);
  }
}
...
#[unsafe(no_mangle)]
unsafe extern "C" fn
void screen_write_insertcharacter(struct screen_write_ctx *ctx, u_int nx,
                                  u_int bg) {
  struct screen *s = (*ctx).s;
  struct tty_ctx ttyctx;
...
#[unsafe(no_mangle)]
unsafe extern "C" fn
void screen_write_deletecharacter(struct screen_write_ctx *ctx, u_int nx,
                                  u_int bg) {
  struct screen *s = (*ctx).s;
  struct tty_ctx ttyctx;
...
#[unsafe(no_mangle)]
unsafe extern "C" fn
void screen_write_clearcharacter(struct screen_write_ctx *ctx, u_int nx,
                                 u_int bg) {
  struct screen *s = (*ctx).s;
  struct tty_ctx ttyctx;
...
#[unsafe(no_mangle)]
unsafe extern "C" fn
void screen_write_cursormove(struct screen_write_ctx *ctx, int px, int py,
                             int origin) {
  struct screen *s = (*ctx).s;
...
#[unsafe(no_mangle)]
unsafe extern "C" fn
void screen_write_scrollregion(struct screen_write_ctx *ctx, u_int rupper,
                               u_int rlower) {
  struct screen *s = (*ctx).s;
...
#[unsafe(no_mangle)]
unsafe extern "C" fn
void screen_write_linefeed(struct screen_write_ctx *ctx, int wrapped,
                           u_int bg) {
  struct screen *s = (*ctx).s;
  struct grid *gd = (*s).grid;
  struct grid_line *gl;
  ...
  log_debug("%s: at %u,%u (region %u-%u)", __func__, (*s).cx, (*s).cy, rupper,
            rlower);
...
#[unsafe(no_mangle)]
unsafe extern "C" fn
void screen_write_scrollup(struct screen_write_ctx *ctx, u_int lines,
                           u_int bg) {
  struct screen *s = (*ctx).s;
  struct grid *gd = (*s).grid;
  u_int i;
...
#[unsafe(no_mangle)]
unsafe extern "C" fn
void screen_write_scrolldown(struct screen_write_ctx *ctx, u_int lines,
                             u_int bg) {
  struct screen *s = (*ctx).s;
  struct grid *gd = (*s).grid;
  struct tty_ctx ttyctx;
...
void screen_write_clearscreen(struct screen_write_ctx *ctx, u_int bg) {
  ...
  if (((*(*s).grid).flags & GRID_HISTORY) && (*ctx).wp != NULL &&
      options_get_number((*(*ctx).wp).options, "scroll-on-clear")) {
...
#[unsafe(no_mangle)]
unsafe extern "C" fn
void screen_write_fullredraw(struct screen_write_ctx *ctx) {
  struct tty_ctx ttyctx;
...
screen_write_collect_trim(struct screen_write_ctx *ctx, u_int y, u_int x,
  ...
  TAILQ_FOREACH_SAFE(ci, &(*cl).items, entry, tmp) {
    ...
    log_debug("%s: %p now %u-%u (%p) and %u-%u (%p)", __func__, ci, (*ci).x,
              (*ci).x + (*ci).used - 1, ci, (*ci2).x,
...
#[unsafe(no_mangle)]
unsafe extern "C" fn
static void screen_write_collect_clear(struct screen_write_ctx *ctx, u_int y,
                                       u_int n) {
...
#[unsafe(no_mangle)]
unsafe extern "C" fn
static void screen_write_collect_flush(struct screen_write_ctx *ctx,
                                       int scroll_only, const char *from) {
  struct screen *s = (*ctx).s;
...
void screen_write_collect_end(struct screen_write_ctx *ctx) {
  ...
  before =
      screen_write_collect_trim(ctx, (*s).cy, (*s).cx, (*ci).used, &wrapped);
  (*ci).x = (*s).cx;
  (*ci).wrapped = wrapped;
  if (before == NULL) {
  ...
  log_debug("%s: %u %.*s (at %u,%u)", __func__, (*ci).used, (int)(*ci).used,
            (*cl).data + (*ci).x, (*s).cx, (*s).cy);
...
#ifdef ENABLE_SIXEL
  if (image_check_area(s, (*s).cx, (*s).cy, (*ci).used, 1) &&
      (*ctx).wp != NULL) {
    (*(*ctx).wp).flags |= PANE_REDRAW;
  }
#endif
...
#[unsafe(no_mangle)]
unsafe extern "C" fn
void screen_write_collect_add(struct screen_write_ctx *ctx,
  ...
  (*(*ctx).s).write_list[(*s).cy].data[(*s).cx + (*ci).used++] =
      (*gc).data.data[0];
}
...
#[unsafe(no_mangle)]
unsafe extern "C" fn
static int screen_write_combine(struct screen_write_ctx *ctx,
                                const struct grid_cell *gc) {
  struct screen *s = (*ctx).s;
  struct grid *gd = (*s).grid;
  const struct utf8_data *ud = &(*gc).data;
  ...
  log_debug("%s: character %.*s at %u,%u (width %u)", __func__, (int)(*ud).size,
            (*ud).data, cx, cy, (*ud).width);
...
static int screen_write_overwrite(struct screen_write_ctx *ctx,
                                  struct grid_cell *gc, u_int width) {
  struct screen *s = (*ctx).s;
  struct grid *gd = (*s).grid;
...
#[unsafe(no_mangle)]
unsafe extern "C" fn
void screen_write_setselection(struct screen_write_ctx *ctx, const char *flags,
...
void screen_write_rawstring(struct screen_write_ctx *ctx, u_char *str,
                            u_int len, int allow_invisible_panes) {
  struct tty_ctx ttyctx;
...
#[unsafe(no_mangle)]
unsafe extern "C" fn
void screen_write_sixelimage(struct screen_write_ctx *ctx,
                             struct sixel_image *si, u_int bg) {
  struct screen *s = (*ctx).s;
  struct grid *gd = (*s).grid;
  struct tty_ctx ttyctx;
...
#[unsafe(no_mangle)]
unsafe extern "C" fn
void screen_write_alternateon(struct screen_write_ctx *ctx,
                              struct grid_cell *gc, int cursor) {
  struct tty_ctx ttyctx;
  struct window_pane *wp = (*ctx).wp;
...
#[unsafe(no_mangle)]
unsafe extern "C" fn
screen_write_alternateoff(struct screen_write_ctx *ctx, struct grid_cell *gc, int cursor) {
  struct tty_ctx ttyctx;
  struct window_pane *wp = (*ctx).wp;
