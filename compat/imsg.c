...
void imsg_init(struct imsgbuf *imsgbuf, int fd) {
...
ssize_t imsg_read(struct imsgbuf *imsgbuf) {
  struct msghdr msg;
  struct cmsghdr *cmsg;
  union {
    struct cmsghdr hdr;
...
again:
  ...
  for (cmsg = CMSG_FIRSTHDR(&msg); cmsg != NULL;
    ...
    if (cmsg->cmsg_level == SOL_SOCKET && cmsg->cmsg_type == SCM_RIGHTS) {
       ...
       */
      j = ((char *)cmsg + cmsg->cmsg_len - (char *)CMSG_DATA(cmsg)) /
          sizeof(int);
      for (i = 0; i < j; i++) {
        fd = ((int *)CMSG_DATA(cmsg))[i];
        if (ifd != NULL) {
          ifd->fd = fd;
          TAILQ_INSERT_TAIL(&imsgbuf->fds, ifd, entry);
          ifd = NULL;
        } else {
          close(fd);
        }
...
ssize_t imsg_get(struct imsgbuf *imsgbuf, struct imsg *imsg) {
  struct imsg m;
  ...
  if (m.hdr.len < IMSG_HEADER_SIZE || m.hdr.len > MAX_IMSGSIZE) {
    errno = ERANGE;
    return -1;
  }
  if (m.hdr.len > av) {
...
int imsg_get_fd(struct imsg *imsg) {
  int fd = imsg->fd;
...
uint32_t imsg_get_id(struct imsg *imsg) { return imsg->hdr.peerid; }

size_t imsg_get_len(struct imsg *imsg) {
  if (imsg->buf == NULL) {
    return 0;
  }
...
pid_t imsg_get_pid(struct imsg *imsg) { return imsg->hdr.pid; }

uint32_t imsg_get_type(struct imsg *imsg) { return imsg->hdr.type; }

int imsg_compose(struct imsgbuf *imsgbuf, uint32_t type, uint32_t id, pid_t pid,
...
int imsg_composev(struct imsgbuf *imsgbuf, uint32_t type, uint32_t id,
 ...
 * with this function.
 */
int imsg_compose_ibuf(struct imsgbuf *imsgbuf, uint32_t type, uint32_t id,
 ...
 */
int imsg_forward(struct imsgbuf *imsgbuf, struct imsg *msg) {
  struct ibuf *wbuf;
  size_t len = 0;
  ...
  if ((wbuf = imsg_create(imsgbuf, msg->hdr.type, msg->hdr.peerid, msg->hdr.pid,
                          len)) == NULL) {
    return -1;
  }
...
struct ibuf *imsg_create(struct imsgbuf *imsgbuf, uint32_t type, uint32_t id,
...
int imsg_add(struct ibuf *msg, const void *data, size_t datalen) {
  if (datalen) {
...
void imsg_close(struct imsgbuf *imsgbuf, struct ibuf *msg) {
  struct imsg_hdr *hdr;
...
void imsg_free(struct imsg *imsg) { ibuf_free(imsg->buf); }

static int imsg_dequeue_fd(struct imsgbuf *imsgbuf) {
  int fd;
  struct imsg_fd *ifd;
...
int imsg_flush(struct imsgbuf *imsgbuf) {
...
void imsg_clear(struct imsgbuf *imsgbuf) {
  int fd;
