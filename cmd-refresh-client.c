...
static void cmd_refresh_client_update_subscription(struct client *tc,
                                                   const char *value) {
...
cmd_refresh_client_control_client_size(struct cmd *self,
  ...
  if (sscanf(size, "@%u:%ux%u", &w, &x, &y) == 3) {
    if (x < WINDOW_MINIMUM || x > WINDOW_MAXIMUM || y < WINDOW_MINIMUM ||
        y > WINDOW_MAXIMUM) {
      cmdq_error(item, "size too small or too big");
      return CMD_RETURN_ERROR;
    }
    log_debug("%s: client %s window @%u: size %ux%u", __func__, tc->name, w, x,
              y);
  ...
  if (sscanf(size, "@%u:", &w) == 1) {
    ...
    if (cw != NULL) {
      log_debug("%s: client %s window @%u: no size", __func__, tc->name, w);
      cw->sx = 0;
      cw->sy = 0;
      recalculate_sizes_now(1);
    }
...
static void cmd_refresh_client_update_offset(struct client *tc,
                                             const char *value) {
...
static enum cmd_retval cmd_refresh_client_clipboard(struct cmd *self,
                                                    struct cmdq_item *item) {
  struct args *args = cmd_get_args(self);
  struct client *tc = cmdq_get_target_client(item);
  const char *p;
  ...
  if (p == NULL) {
    ...
    tc->clipboard_panes =
        xreallocarray(tc->clipboard_panes, tc->clipboard_npanes + 1,
                      sizeof *tc->clipboard_panes);
...
static void cmd_refresh_report(struct tty *tty, const char *value) {
  struct window_pane *wp;
  u_int pane;
  size_t size = 0;
  char *copy, *split;
...
static enum cmd_retval cmd_refresh_client_exec(struct cmd *self,
                                               struct cmdq_item *item) {
  struct args *args = cmd_get_args(self);
  struct client *tc = cmdq_get_target_client(item);
  ...
  if (args_has(args, 'c') || args_has(args, 'L') || args_has(args, 'R') ||
      args_has(args, 'U') || args_has(args, 'D')) {
