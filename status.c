...
static const char *prompt_type_strings[] = {"command", "search", "target",
                                            "window-target"};
...
static void status_prompt_add_typed_history(char *line) {
  char *typestr;
...
static void status_timer_callback(__unused int fd, __unused short events,
                                  void *arg) {
  struct client *c = arg;
...
static u_int status_prompt_line_at(struct client *c) {
  struct session *s = c->session;
...
static void status_message_callback(__unused int fd, __unused short event,
                                    void *data) {
...
void status_prompt_set(struct client *c, struct cmd_find_state *fs,
                       const char *msg, const char *input,
                       prompt_input_cb inputcb, prompt_free_cb freecb,
                       void *data, int flags, enum prompt_type prompt_type) {
  struct format_tree *ft;
  char *tmp;
...
void status_prompt_update(struct client *c, const char *msg,
                          const char *input) {
  struct format_tree *ft;
  char *tmp;
 ...
 */
static int status_prompt_translate_key(struct client *c, key_code key,
                                       key_code *new_key) {
...
static int status_prompt_paste(struct client *c) {
  ...
  if (n != 0) {
    c->prompt_buffer =
        xreallocarray(c->prompt_buffer, size + n + 1, sizeof *c->prompt_buffer);
...
static int status_prompt_replace_complete(struct client *c, const char *s) {
  ...
  /* Try to complete it. */
  if (s == NULL) {
    allocated = status_prompt_complete(c, word, first - c->prompt_buffer);
    if (allocated == NULL) {
      return 0;
    }
    s = allocated;
  ...
  c->prompt_buffer =
      xreallocarray(c->prompt_buffer, size + 1, sizeof *c->prompt_buffer);
...
static void status_prompt_forward_word(struct client *c, size_t size, int vi,
  ...
  if (!vi) {
    while (idx != size && status_prompt_space(&c->prompt_buffer[idx])) {
      idx++;
    }
  }
...
static void status_prompt_end_word(struct client *c, size_t size,
                                   const char *separators) {
  size_t idx = c->prompt_index;
  int word_is_separators;
...
static void status_prompt_backward_word(struct client *c,
                                        const char *separators) {
  size_t idx = c->prompt_index;
  int word_is_separators;
...
process_key:
  ...
  case 'w' | KEYC_CTRL:
    ...
    free(c->prompt_saved);
    c->prompt_saved =
        xcalloc(sizeof *c->prompt_buffer, (c->prompt_index - idx) + 1);
    memcpy(c->prompt_saved, c->prompt_buffer + idx,
           (c->prompt_index - idx) * sizeof *c->prompt_buffer);

    memmove(c->prompt_buffer + idx, c->prompt_buffer + c->prompt_index,
            (size + 1 - c->prompt_index) * sizeof *c->prompt_buffer);
    memset(c->prompt_buffer + size - (c->prompt_index - idx), '\0',
           (c->prompt_index - idx) * sizeof *c->prompt_buffer);
    c->prompt_index = idx;
  ...
  case KEYC_UP:
  case 'p' | KEYC_CTRL:
    histstr = status_prompt_up_history(c->prompt_hindex, c->prompt_type);
    if (histstr == NULL) {
      break;
    }
    free(c->prompt_buffer);
  ...
  case KEYC_DOWN:
  case 'n' | KEYC_CTRL:
    histstr = status_prompt_down_history(c->prompt_hindex, c->prompt_type);
    if (histstr == NULL) {
      break;
    }
    free(c->prompt_buffer);
...
append_key:
  ...
  c->prompt_buffer =
      xreallocarray(c->prompt_buffer, size + 2, sizeof *c->prompt_buffer);
...
static const char *status_prompt_up_history(u_int *idx, u_int type) {
  /*
...
static const char *status_prompt_down_history(u_int *idx, u_int type) {
...
static void status_prompt_add_history(const char *line, u_int type) {
  ...
  if (hlimit > oldsize) {
    ...
    movesize = (oldsize - freecount) * sizeof *status_prompt_hlist[type];
    if (movesize > 0) {
  ...
  if (newsize == 0) {
    ...
    status_prompt_hlist[type] = xreallocarray(
        status_prompt_hlist[type], newsize, sizeof *status_prompt_hlist[type]);
  }
...
static void status_prompt_add_list(char ***list, u_int *size, const char *s) {
  u_int i;
...
static char **status_prompt_complete_list(u_int *size, const char *s,
                                          int at_start) {
  char **list = NULL, *tmp;
  ...
  const char *layouts[] = {"even-horizontal", "even-vertical",
  ...
  *size = 0;
  for (cmdent = cmd_table; *cmdent != NULL; cmdent++) {
    ...
    if ((*cmdent)->alias != NULL && strncmp((*cmdent)->alias, s, slen) == 0) {
...
static void status_prompt_menu_callback(__unused struct menu *menu, u_int idx,
...
static int status_prompt_complete_list_menu(struct client *c, char **list,
...
/* Show complete word menu. */
static char *status_prompt_complete_window_menu(struct client *c,
                                                struct session *s,
                                                const char *word, u_int offset,
                                                char flag) {
  struct menu *menu;
  struct menu_item item;
  struct status_prompt_menu *spm;
  ...
  RB_FOREACH(wl, winlinks, &s->windows) {
    ...
    if (c->prompt_type == PROMPT_TYPE_WINDOW_TARGET) {
      ...
      xasprintf(&tmp, "%s:%d (%s)", s->name, wl->idx, wl->window->name);
...
/* Complete a session. */
static char *status_prompt_complete_session(char ***list, u_int *size,
                                            const char *s, char flag) {
  struct session *loop;
  char *out, *tmp, n[11];
  ...
  RB_FOREACH(loop, sessions, &sessions) {
    if (*s == '\0' || strncmp(loop->name, s, strlen(s)) == 0) {
      *list = xreallocarray(*list, (*size) + 2, sizeof **list);
      ...
      if (s[1] == '\0' || strncmp(n, s + 1, strlen(s) - 1) == 0) {
        *list = xreallocarray(*list, (*size) + 2, sizeof **list);
...
/* Complete word. */
static char *status_prompt_complete(struct client *c, const char *word,
                                    u_int offset) {
  ...
  if (*word == '\0' && c->prompt_type != PROMPT_TYPE_TARGET &&
  ...
  if (c->prompt_type != PROMPT_TYPE_TARGET &&
      c->prompt_type != PROMPT_TYPE_WINDOW_TARGET &&
      strncmp(word, "-t", 2) != 0 && strncmp(word, "-s", 2) != 0) {
  ...
  if (c->prompt_type == PROMPT_TYPE_WINDOW_TARGET) {
    out = status_prompt_complete_window_menu(c, c->session, s, offset, '\0');
    goto found;
  }
  colon = strchr(s, ':');
  ...
  if (strchr(colon + 1, '.') == NULL) {
    ...
    out =
        status_prompt_complete_window_menu(c, session, colon + 1, offset, flag);
    if (out == NULL) {
      return NULL;
    }
  }
...
const char *status_prompt_type_string(u_int type) {
