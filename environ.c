...
int environ_cmp(struct environ_entry *envent1, struct environ_entry *envent2);
// int environ_cmp(struct environ_entry *envent1, struct environ_entry *envent2)
...
// struct environ_entry * environ_first(struct environ *env) { return
// (RB_MIN(environ, env)); }

struct environ_entry *environ_next(struct environ_entry *envent);
// struct environ_entry * environ_next(struct environ_entry *envent) { return
// (RB_NEXT(environ, env, envent)); }
...
void environ_copy(struct environ *srcenv, struct environ *dstenv) {
  ...
  RB_FOREACH(envent, environ, srcenv) {
    if (envent->value == NULL) {
      ...
      environ_set(dstenv, envent->name, envent->flags, "%s", envent->value);
    }
  }
}
...
void environ_update(struct options *oo, struct environ *src,
                    struct environ *dst) {
  struct environ_entry *envent;
