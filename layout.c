...
static u_int layout_new_pane_size(struct window *, u_int, struct layout_cell *,
...
void layout_print_cell(struct layout_cell *lc, const char *hdr, u_int n) {
  ...
  log_debug("%s:%*s%p type %s [parent %p] wp=%p [%u,%u %ux%u]", hdr, n, " ", lc,
            type, lc->parent, lc->wp, lc->xoff, lc->yoff, lc->sx, lc->sy);
  switch (lc->type) {
  case LAYOUT_LEFTRIGHT:
  case LAYOUT_TOPBOTTOM:
