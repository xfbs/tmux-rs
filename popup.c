...
static struct screen *popup_mode_cb(__unused struct client *c, void *data,
                                    u_int *cx, u_int *cy) {
...
static void popup_check_cb(struct client *c, void *data, u_int px, u_int py,
  ...
  if (pd->md != NULL) {
    ...
    for (i = 0; i < 2; i++) {
      server_client_overlay_range(pd->px, pd->py, pd->sx, pd->sy, r->px[i], py,
                                  r->nx[i], &or [i]);
    }
...
static void popup_draw_cb(struct client *c, void *data,
  ...
  if (pd->border_lines == BOX_LINES_NONE) {
    ...
    screen_write_box(&ctx, pd->sx, pd->sy, pd->border_lines, &pd->border_cell,
                     pd->title);
  ...
  for (i = 0; i < pd->sy; i++) {
    tty_draw_line(tty, &s, 0, i, pd->sx, px, py + i, &defaults, palette);
  }
  screen_free(&s);
  if (pd->md != NULL) {
    c->overlay_check = NULL;
...
static void popup_free_cb(struct client *c, void *data) {
  struct popup_data *pd = data;
...
static void popup_resize_cb(__unused struct client *c, void *data) {
  struct popup_data *pd = data;
  struct tty *tty = &c->tty;
...
static void popup_make_pane(struct popup_data *pd, enum layout_type type) {
  ...
  new_wp->fd =
      job_transfer(pd->job, &new_wp->pid, new_wp->tty, sizeof new_wp->tty);
  pd->job = NULL;
...
static void popup_menu_done(__unused struct menu *menu, __unused u_int choice,
...
static void popup_handle_drag(struct client *c, struct popup_data *pd,
...
static int popup_key_cb(struct client *c, void *data, struct key_event *event) {
  struct popup_data *pd = data;
  ...
  if (KEYC_IS_MOUSE(event->key)) {
    ...
    if (m->x < pd->px || m->x > pd->px + pd->sx - 1 || m->y < pd->py ||
        m->y > pd->py + pd->sy - 1) {
...
menu:
  ...
  if (pd->flags & POPUP_INTERNAL) {
    menu_add_items(pd->menu, popup_internal_menu_items, NULL, c, NULL);
  } else {
  ...
  pd->md = menu_prepare(pd->menu, 0, 0, NULL, x, m->y, c, BOX_LINES_DEFAULT,
                        NULL, NULL, NULL, NULL, popup_menu_done, pd);
...
int popup_display(int flags, enum box_lines lines, struct cmdq_item *item,
                  u_int px, u_int py, u_int sx, u_int sy, struct environ *env,
                  const char *shellcmd, int argc, char **argv, const char *cwd,
                  const char *title, struct client *c, struct session *s,
  ...
  pd->job =
      job_run(shellcmd, argc, argv, env, s, cwd, popup_job_update_cb,
              popup_job_complete_cb, NULL, pd,
              JOB_NOWAIT | JOB_PTY | JOB_KEEPWRITE | JOB_DEFAULTSHELL, jx, jy);
  pd->ictx = input_init(NULL, job_get_event(pd->job), &pd->palette);

  server_client_set_overlay(c, 0, popup_check_cb, popup_mode_cb, popup_draw_cb,
                            popup_key_cb, popup_free_cb, popup_resize_cb, pd);
  return 0;
}
...
static void popup_editor_close_cb(int status, void *arg) {
  ...
  if (f != NULL) {
    ...
    if (len == 0 || (uintmax_t)len > (uintmax_t)SIZE_MAX ||
        (buf = malloc(len)) == NULL || fread(buf, len, 1, f) != 1) {
      free(buf);
      buf = NULL;
      len = 0;
    }
...
int popup_editor(struct client *c, const char *buf, size_t len,
  ...
  if (popup_display(POPUP_INTERNAL | POPUP_CLOSEEXIT, BOX_LINES_DEFAULT, NULL,
                    px, py, sx, sy, NULL, cmd, 0, NULL, _PATH_TMP, NULL, c,
