...
static enum args_parse_type cmd_bind_key_args_parse(__unused struct args *args,
                                                    __unused u_int idx,
                                                    __unused char **cause) {
  return (ARGS_PARSE_COMMANDS_OR_STRING);
}

static enum cmd_retval cmd_bind_key_exec(struct cmd *self,
  ...
  if (count == 2)
    ...
    pr = cmd_parse_from_arguments(args_values(args) + 1, count - 1, NULL);
  }
  switch (pr->status) {
