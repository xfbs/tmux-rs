...
pub unsafe fn tty_client_ready(ctx: *const tty_ctx, c: *mut client) -> bool {
    unsafe {
        if client_get_session(c).is_null() || (*c).tty.term.is_null() {
            return false;
        }
...
pub unsafe fn tty_default_colours(gc: *mut grid_cell, wp: *mut window_pane) {
    unsafe {
        ...
        if (*gc).fg == 8 {
            if wp == window_active_pane(window_pane_window(wp)) && (*wp).cached_active_gc.fg != 8 {
        ...
        if (*gc).bg == 8 {
            if wp == window_active_pane(window_pane_window(wp)) && (*wp).cached_active_gc.bg != 8 {
