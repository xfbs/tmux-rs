...
pub unsafe fn window_copy_cmd_previous_matching_bracket(
    ...
    unsafe {
        ...
        'outer: while np != 0 {
            ...
            let mut tried = false;
            'retry: loop {
                gc = (*s).grid.get_cell(px, py);
                if gc.data.size != 1 || gc.flags.intersects(GridFlag::PADDING) {
                    cp = null_mut();
                } else {
            ...
            'retry: loop {
                ...
                if gc.data.size != 1 || gc.flags.intersects(GridFlag::PADDING) {
                    ...
                    if !cp.is_null() && (*data).modekeys == modekey::MODEKEY_VI {
                        ...
                        gc = (*s).grid.get_cell(px, py);
                        if gc.data.size == 1
                            && !gc.flags.intersects(GridFlag::PADDING)
...
pub unsafe fn window_copy_search_compare(
    ...
    unsafe {
        let gc: GridCell = (*gd).get_cell(px, py);
        let ud = &raw const gc.data;
        let sgc: GridCell = (*sgd).get_cell(spx, 0);
...
pub unsafe fn window_copy_cellstring(
    ...
    unsafe {
        ...
        if gce.flags.intersects(GridFlag::PADDING) {
            *size = 0;
            *allocated = 0;
            return null_mut();
        }
...
pub unsafe fn window_copy_match_at_cursor(data: *mut window_copy_mode_data) -> Option<String> {
    unsafe {
        ...
        for at in start..=end {
            ...
            let gc = (*gd).get_cell(px, (*gd).hsize + py - (*data).oy);
...
pub unsafe fn window_copy_write_line(
    ...
    unsafe {
        ...
        let gl: *mut GridLine;
        let mut gc: GridCell = zeroed();
        let mut mgc: GridCell = zeroed();
        let mut cgc: GridCell = zeroed();
        let mut mkgc: GridCell = zeroed();
...
pub unsafe fn window_copy_copy_line(
    ...
    unsafe {
        ...
        if (*gl).flags.intersects(GridLineFlag::WRAPPED) && (*gl).celldata.len() as u32 <= (*gd).sx {
            wrapped = true;
        }
...
pub unsafe fn window_copy_cursor_start_of_line(wme: *mut window_mode_entry) {
    unsafe {
        ...
        let mut gr = GridReader::new(&mut *(*back_s).grid, px, py);
...
pub unsafe fn window_copy_cursor_next_word(wme: *mut window_mode_entry, separators: *const u8) {
    unsafe {
        ...
        let sep = CStr::from_ptr(separators.cast());
        let mut gr = GridReader::new(&mut *(*back_s).grid, px, py);
        gr.cursor_next_word(sep);
        (px, py) = gr.cursor();
        window_copy_acquire_cursor_down(
            wme,
            hsize,
...
pub unsafe fn window_copy_cursor_previous_word(
    ...
    unsafe {
        ...
        let sep = CStr::from_ptr(separators.cast());
        let mut gr = GridReader::new(&mut *(*back_s).grid, px, py);
        gr.cursor_previous_word(sep, already, stop_at_eol);
        (px, py) = gr.cursor();
        window_copy_acquire_cursor_up(wme, hsize, (*data).oy, oldy, px, py);
    }
}
...
pub unsafe fn window_copy_cursor_prompt(
    ...
    unsafe {
        ...
        let line_flag = if !args.is_null() && streq_(args, "-o") {
            GridLineFlag::START_OUTPUT
        } else {
            GridLineFlag::START_PROMPT
        };
