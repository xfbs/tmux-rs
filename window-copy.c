...
window_copy_cmd_select_word(struct window_copy_cmd_state *cs) {
  ...
  nextx = px + 1;
  nexty = py;
  if (grid_get_line(data->backing->grid, nexty)->flags & GRID_LINE_WRAPPED &&
...
static int window_copy_search_marks(struct window_mode_entry *wme,
  ...
  if (regex) {
    sbuf = xmalloc(ssize);
    sbuf[0] = '\0';
    sbuf = window_copy_stringify(ssp->grid, 0, 0, ssp->grid->sx, sbuf, &ssize);
    if (cis) {
      cflags |= REG_ICASE;
    }
    if (regcomp(&reg, sbuf, cflags) != 0) {
...
static void *window_copy_get_selection(struct window_mode_entry *wme,
  ...
  *buf = '\0';
  ...
  if (keys == MODEKEY_EMACS || lastex <= ey_last) {
    if (~grid_get_line(data->backing->grid, ey)->flags & GRID_LINE_WRAPPED ||
        lastex != ey_last) {
      off -= 1;
    }
  }
