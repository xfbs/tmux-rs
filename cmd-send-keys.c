...
const struct cmd_entry cmd_send_keys_entry = {
    ...
    .usage =
        "[-FHKlMRX] [-c target-client] [-N repeat-count] " CMD_TARGET_PANE_USAGE
        " key ...",
...
static struct cmdq_item *cmd_send_keys_inject_key(struct cmdq_item *item,
...
static struct cmdq_item *cmd_send_keys_inject_string(struct cmdq_item *item,
                                                     struct cmdq_item *after,
  ...
  if (args_has(args, 'H')) {
    ...
    return cmd_send_keys_inject_key(item, after, args, KEYC_LITERAL | n);
  }
...
static enum cmd_retval cmd_send_keys_exec(struct cmd *self,
  ...
  if (args_has(args, 'N')) {
    np = args_strtonum_and_expand(args, 'N', 1, UINT_MAX, item, &cause);
    if (cause != NULL) {
