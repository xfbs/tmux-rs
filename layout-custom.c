...
static int layout_append(struct layout_cell *lc, char *buf, size_t len) {
  ...
  if (lc->wp != NULL) {
    tmplen = xsnprintf(tmp, sizeof tmp, "%ux%u,%u,%u,%u", lc->sx, lc->sy,
                       lc->xoff, lc->yoff, lc->wp->id);
  } else {
    tmplen = xsnprintf(tmp, sizeof tmp, "%ux%u,%u,%u", lc->sx, lc->sy, lc->xoff,
                       lc->yoff);
  }
...
int layout_parse(struct window *w, const char *layout, char **cause) {
...
static struct layout_cell *layout_construct(struct layout_cell *lcparent,
                                            const char **layout) {
  struct layout_cell *lc, *lcchild;
