...
struct session *session_create(const char *prefix, const char *name,
                               const char *cwd, struct environ *env,
                               struct options *oo, struct termios *tio) {
  struct session *s;
...
/* Free session. */
static void session_free(__unused int fd, __unused short events, void *arg) {
...
static void session_lock_timer(__unused int fd, __unused short events,
                               void *arg) {
  struct session *s = arg;
 ...
 * stack and alerts.
 */
static void session_group_synchronize1(struct session *target,
                                       struct session *s) {
  ...
  if (s->curw != NULL && winlink_find_by_index(ww, s->curw->idx) == NULL &&
