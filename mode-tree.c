...
static void mode_tree_build_lines(struct mode_tree_data *mtd,
  ...
  mtd->depth = depth;
  TAILQ_FOREACH(mti, mtl, entry) {
    mtd->line_list = xreallocarray(mtd->line_list, mtd->line_size + 1,
                                   sizeof *mtd->line_list);
    ...
    if (mtd->keycb != NULL) {
      mti->key = mtd->keycb(mtd->modedata, mti->itemdata, mti->line);
    ...
    if (mti->key != KEYC_NONE) {
      mti->keystr = xstrdup(key_string_lookup_key(mti->key, 0));
...
int mode_tree_down(struct mode_tree_data *mtd, int wrap) {
...
static int mode_tree_get_tag(struct mode_tree_data *mtd, uint64_t tag,
                             u_int *found) {
  u_int i;
...
void mode_tree_each_tagged(struct mode_tree_data *mtd, mode_tree_each_cb cb,
...
mode_tree_start(struct window_pane *wp, struct args *args,
                ...
                void *modedata, const struct menu_item *menu,
...
struct mode_tree_item *mode_tree_add(struct mode_tree_data *mtd,
                                     struct mode_tree_item *parent,
                                     void *itemdata, uint64_t tag,
                                     const char *name, const char *text,
                                     int expanded) {
  struct mode_tree_item *mti, *saved;

  log_debug("%s: %llu, %s %s", __func__, (unsigned long long)tag, name,
            (text == NULL ? "" : text));
...
void mode_tree_no_tag(struct mode_tree_item *mti) { mti->no_tag = 1; }
  ...
  for (i = 0; i < mtd->line_size; i++) {
    ...
    if (line->depth == 0) {
      ...
      for (j = 1; j < line->depth; j++) {
        if (mti->parent != NULL && mtd->line_list[mti->parent->line].last) {
          strlcat(start, "    ", size);
        } else {
    ...
    if (i != mtd->current) {
      ...
      if (mti->text != NULL) {
        format_draw(&ctx, &gc0, w - width, mti->text, NULL, 0);
      }
    } else {
      screen_write_clearendofline(&ctx, gc.bg);
      screen_write_nputs(&ctx, w, &gc, "%s", text);
      if (mti->text != NULL) {
        format_draw(&ctx, &gc, w - width, mti->text, NULL, 0);
      }
    }
    free(text);
    free(key);
...
static int mode_tree_search_callback(__unused struct client *c, void *data,
...
static void mode_tree_search_free(void *data) { mode_tree_remove_ref(data); }

static int mode_tree_filter_callback(__unused struct client *c, void *data,
                                     const char *s, __unused int done) {
  struct mode_tree_data *mtd = data;
...
static void mode_tree_filter_free(void *data) { mode_tree_remove_ref(data); }

static void mode_tree_menu_callback(__unused struct menu *menu,
                                    __unused u_int idx, key_code key,
                                    void *data) {
...
static void mode_tree_display_menu(struct mode_tree_data *mtd, struct client *c,
                                   u_int x, u_int y, int outside) {
  struct mode_tree_item *mti;
  struct menu *menu;
  ...
  if (menu_display(menu, 0, 0, NULL, x, y, c, BOX_LINES_DEFAULT, NULL, NULL,
                   NULL, NULL, mode_tree_menu_callback, mtm) != 0) {
    menu_free(menu);
  }
}

int mode_tree_key(struct mode_tree_data *mtd, struct client *c, key_code *key,
  ...
  if (KEYC_IS_MOUSE(*key) && m != NULL) {
    ...
    if (mtd->offset + y < mtd->line_size) {
      if (*key == KEYC_MOUSEDOWN1_PANE || *key == KEYC_MOUSEDOWN3_PANE ||
  ...
  case 's' | KEYC_CTRL:
    mtd->references++;
    status_prompt_set(c, NULL, "(search) ", "", mode_tree_search_callback,
                      mode_tree_search_free, mtd, PROMPT_NOFORMAT,
                      PROMPT_TYPE_SEARCH);
    break;
  case 'n':
...
void mode_tree_run_command(struct client *c, struct cmd_find_state *fs,
