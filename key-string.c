...
extern struct { const char *string; key_code key; } key_string_table[469];
...
/*
key_code key_string_search_table(const char *string) {
  u_int i, user;
...
/* Find modifiers. */
/*
key_code key_string_get_modifiers(const char **string) {
  key_code modifiers;
...
key_code key_string_lookup_string(const char *string) {
  ...
  if (string[0] == '0' && string[1] == 'x') {
    ...
    udp = utf8_fromcstr(m);
    if (udp == NULL || udp[0].size == 0 || udp[1].size != 0 ||
