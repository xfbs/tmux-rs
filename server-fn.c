...
void server_status_client(struct client *c) { c->flags |= CLIENT_REDRAWSTATUS; }
...
int server_link_window(struct session *src, struct winlink *srcwl,
                       struct session *dst, int dstidx, int killflag,
                       int selectflag, char **cause) {
...
static struct session *server_find_session(struct session *s,
                                           int (*f)(struct session *,
                                                    struct session *)) {
...
static int server_newer_session(struct session *s_loop, struct session *s_out) {
  return timercmp(&s_loop->activity_time, &s_out->activity_time, >);
}

static int server_newer_detached_session(struct session *s_loop,
