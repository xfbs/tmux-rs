...
int image_free_all(struct screen *s) {
  struct image *im, *im1;
...
static void image_fallback(char **ret, u_int sx, u_int sy) {
  char *buf, *label;
  u_int py, size, lsize;
...
struct image *image_store(struct screen *s, struct sixel_image *si) {
  struct image *im;
...
int image_check_line(struct screen *s, u_int py, u_int ny) {
  struct image *im, *im1;
  int redraw = 0;
...
int image_check_area(struct screen *s, u_int px, u_int py, u_int nx, u_int ny) {
  struct image *im, *im1;
  int redraw = 0;
...
int image_scroll_up(struct screen *s, u_int lines) {
  struct image *im, *im1;
  int redraw = 0;
  u_int sx, sy;
