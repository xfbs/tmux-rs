...
#define CONTROL_IGNORE_FLAGS (CLIENT_CONTROL_NOOUTPUT | CLIENT_UNATTACHEDFLAGS)

/* Compare client panes. */
int control_pane_cmp(struct control_pane *cp1, struct control_pane *cp2);
/*
static int control_pane_cmp(struct control_pane *cp1, struct control_pane *cp2)
...
int control_sub_cmp(struct control_sub *csub1, struct control_sub *csub2);
/*
static int control_sub_cmp(struct control_sub *csub1, struct control_sub
*csub2) { return strcmp(csub1->name, csub2->name);
}
*/
RB_GENERATE_STATIC(control_subs, control_sub, entry, control_sub_cmp);

/* Compare client subscription panes. */
int control_sub_pane_cmp(struct control_sub_pane *csp1,
                         struct control_sub_pane *csp2);
/*
static int control_sub_pane_cmp(struct control_sub_pane *csp1, struct
control_sub_pane *csp2) { if (csp1->pane < csp2->pane) { return -1;
  }
...
int control_sub_window_cmp(struct control_sub_window *csw1,
                           struct control_sub_window *csw2);
/*
static int control_sub_window_cmp(struct control_sub_window *csw1, struct
control_sub_window *csw2) { if (csw1->window < csw2->window) { return -1;
  }
...
/* Free a subscription. */
void control_free_sub(struct control_state *cs, struct control_sub *csub);
/*
void control_free_sub(struct control_state *cs, struct control_sub *csub) {
...
/* Free a block. */
void control_free_block(struct control_state *cs, struct control_block *cb);
/*
void control_free_block(struct control_state *cs, struct control_block *cb)
{
  free(cb->line);
...
struct control_pane *control_get_pane(struct client *c, struct window_pane *wp);
/*
struct control_pane *control_get_pane(struct client *c, struct window_pane *wp)
{ struct control_state *cs = c->control_state; struct control_pane cp = {.pane =
wp->id};
...
struct control_pane *control_add_pane(struct client *c, struct window_pane *wp);
/*
struct control_pane *control_add_pane(struct client *c, struct window_pane *wp)
{ struct control_state *cs = c->control_state; struct control_pane *cp;
...
void control_discard_pane(struct client *c, struct control_pane *cp);
/*
void control_discard_pane(struct client *c, struct control_pane *cp) {
...
struct window_pane *control_window_pane(struct client *c, u_int pane);
/*
struct window_pane *control_window_pane(struct client *c, u_int pane) {
...
void control_reset_offsets(struct client *c);
/*
void control_reset_offsets(struct client *c) {
  struct control_state *cs = c->control_state;
...
/*
struct window_pane_offset * control_pane_offset(struct client *c, struct
window_pane *wp, int *off)
{
  struct control_state *cs = c->control_state;
...
/* Set pane as on. */
void control_set_pane_on(struct client *c, struct window_pane *wp);
/*
void control_set_pane_on(struct client *c, struct window_pane *wp) {
  struct control_pane *cp;
...
void control_set_pane_off(struct client *c, struct window_pane *wp);
/*
void control_set_pane_off(struct client *c, struct window_pane *wp) {
  struct control_pane *cp;
...
void control_continue_pane(struct client *c, struct window_pane *wp);
/*
void control_continue_pane(struct client *c, struct window_pane *wp) {
  struct control_pane *cp;
...
/* Pause a pane. */
void control_pause_pane(struct client *c, struct window_pane *wp);
/*
void control_pause_pane(struct client *c, struct window_pane *wp) {
  struct control_pane *cp;
...
/* Write a line. */
void control_vwrite(struct client *c, const char *fmt, va_list ap);
/*
void control_vwrite(struct client *c, const char *fmt, va_list ap) {
  struct control_state *cs = c->control_state;
  char *s;
...
int control_check_age(struct client *c, struct window_pane *wp,
                      struct control_pane *cp);
/*
int control_check_age(struct client *c, struct window_pane *wp, struct
control_pane *cp) { struct control_block *cb; uint64_t t, age;
...
void control_write_output(struct client *c, struct window_pane *wp) {
  ...
  if (!cp->pending_flag) {
    log_debug("%s: %s: %%%u now pending", __func__, c->name, wp->id);
...
enum cmd_retval control_error(struct cmdq_item *item, void *data);
/*
enum cmd_retval control_error(struct cmdq_item *item, void *data) {
...
void control_error_callback(__unused struct bufferevent *bufev,
                            __unused short what, void *data);
/*
void control_error_callback(__unused struct bufferevent *bufev, __unused short
what, void *data) { struct client *c = data;
...
void control_read_callback(__unused struct bufferevent *bufev, void *data);
/*
void control_read_callback(__unused struct bufferevent *bufev, void *data) {
...
int control_all_done(struct client *c);
/*
int control_all_done(struct client *c) {
  struct control_state *cs = c->control_state;
...
void control_flush_all_blocks(struct client *c);
/*
void control_flush_all_blocks(struct client *c) {
  struct control_state *cs = c->control_state;
  struct control_block *cb, *cb1;
  ...
  TAILQ_FOREACH_SAFE(cb, &cs->all_blocks, all_entry, cb1) {
    ...
    log_debug("%s: %s: flushing line: %s", __func__, c->name, cb->line);
...
struct evbuffer *control_append_data(struct client *c, struct control_pane *cp,
                                     uint64_t age, struct evbuffer *message,
                                     struct window_pane *wp, size_t size);
/*
struct evbuffer * control_append_data(struct client *c, struct control_pane *cp,
uint64_t age, struct evbuffer *message, struct window_pane *wp, size_t size) {
  u_char *new_data;
  size_t new_size;
  u_int i;
  ...
  if (message == NULL) {
    ...
    if (c->flags & CLIENT_CONTROL_PAUSEAFTER) {
      evbuffer_add_printf(message, "%%extended-output %%%u %llu : ", wp->id,
                          (unsigned long long)age);
    } else {
...
/* Write buffer. */
void control_write_data(struct client *c, struct evbuffer *message);
/*
void control_write_data(struct client *c, struct evbuffer *message) {
  struct control_state *cs = c->control_state;

  log_debug("%s: %s: %.*s", __func__, c->name, (int)EVBUFFER_LENGTH(message),
...
int control_write_pending(struct client *c, struct control_pane *cp,
                          size_t limit);
/*
int control_write_pending(struct client *c, struct control_pane *cp, size_t
limit) { struct control_state *cs = c->control_state; struct window_pane *wp =
NULL; struct evbuffer *message = NULL; size_t used = 0, size; struct
control_block *cb, *cb1; uint64_t age, t = get_timer();
  ...
  while (used != limit && !TAILQ_EMPTY(&cp->blocks)) {
    ...
    log_debug("%s: %s: output block %zu (age %llu) for %%%u "
              "(used %zu/%zu)",
              __func__, c->name, cb->size, (unsigned long long)age, cp->pane,
              used, limit);
...
void control_write_callback(__unused struct bufferevent *bufev, void *data);
/*
void control_write_callback(__unused struct bufferevent *bufev, void *data) {
  struct client *c = data;
  struct control_state *cs = c->control_state;
  ...
  while (EVBUFFER_LENGTH(evb) < CONTROL_BUFFER_HIGH) {
    ...
    log_debug("%s: %s: %zu bytes available, %u panes", __func__, c->name, space,
              cs->pending_count);
...
void control_start(struct client *c);
/*
void control_start(struct client *c) {
  ...
  cs->read_event =
      bufferevent_new(c->fd, control_read_callback, control_write_callback,
  ...
  if (c->flags & CLIENT_CONTROLCONTROL) {
    ...
    cs->write_event = bufferevent_new(c->out_fd, NULL, control_write_callback,
                                      control_error_callback, c);
    if (cs->write_event == NULL) {
      fatalx("out of memory");
    }
  }
  bufferevent_setwatermark(cs->write_event, EV_WRITE, CONTROL_BUFFER_LOW, 0);
...
/* Stop control mode. */
void control_stop(struct client *c);
/*
void control_stop(struct client *c) {
  struct control_state *cs = c->control_state;
  struct control_block *cb, *cb1;
...
void control_check_subs_session(struct client *c, struct control_sub *csub);
/*
void control_check_subs_session(struct client *c, struct control_sub *csub) {
  ...
  control_write(c, "%%subscription-changed %s $%u - - - : %s", csub->name,
                s->id, value);
  free(csub->last);
  csub->last = value;
}
...
void control_check_subs_pane(struct client *c, struct control_sub *csub);
/*
void control_check_subs_pane(struct client *c, struct control_sub *csub) {
  struct session *s = c->session;
  struct window_pane *wp;
...
void control_check_subs_all_panes(struct client *c, struct control_sub *csub);
/*
void control_check_subs_all_panes(struct client *c, struct control_sub *csub) {
  struct session *s = c->session;
  struct window_pane *wp;
...
void control_check_subs_window(struct client *c, struct control_sub *csub);
/*
void control_check_subs_window(struct client *c, struct control_sub *csub) {
  struct session *s = c->session;
  struct window *w;
  ...
  TAILQ_FOREACH(wl, &w->winlinks, wentry) {
    ...
    control_write(c, "%%subscription-changed %s $%u @%u %u - : %s", csub->name,
                  s->id, w->id, wl->idx, value);
    free(csw->last);
    csw->last = value;
  }
}
...
void control_check_subs_all_windows(struct client *c, struct control_sub *csub);
/*
void control_check_subs_all_windows(struct client *c, struct control_sub *csub)
{ struct session *s = c->session; struct window *w; struct winlink *wl; struct
format_tree *ft; char *value; struct control_sub_window *csw, find;
...
void control_check_subs_timer(__unused int fd, __unused short events,
                              void *data);
/*
void control_check_subs_timer(__unused int fd, __unused short events, void
*data) { struct client *c = data; struct control_state *cs = c->control_state;
  struct control_sub *csub, *csub1;
...
/* Add a subscription. */
void control_add_sub(struct client *c, const char *name,
                     enum control_sub_type type, int id, const char *format) {
  struct control_state *cs = c->control_state;
...
void control_remove_sub(struct client *c, const char *name) {
  struct control_state *cs = c->control_state;
  struct control_sub *csub, find;
