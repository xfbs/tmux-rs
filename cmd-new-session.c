...
enum cmd_retval cmd_new_session_exec(struct cmd *, struct cmdq_item *);
...
const struct cmd_entry cmd_new_session_entry = {
    ...
    .usage =
        ...
        "[-f flags] [-n window-name] [-s "
        "session-name] " CMD_TARGET_SESSION_USAGE " [-x width] [-y height] "
        "[shell-command]",
...
// extern struct cmd_entry cmd_has_session_entry;

enum cmd_retval cmd_new_session_exec(struct cmd *self, struct cmdq_item *item) {
  ...
  if (args_has(args, 'A')) {
    ...
    if (as != NULL) {
      retval = cmd_attach_session(item, as->name, args_has(args, 'D'),
  ...
  if (group != NULL) {
    ...
    if (sg != NULL) {
      ...
      if (prefix == NULL) {
        cmdq_error(item, "invalid session group: %s", group);
        goto fail;
      }
    }
  }
