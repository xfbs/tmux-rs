...
static int cmd_find_session_better(struct session *s, struct session *than,
                                   int flags) {
  int attached;
...
static int cmd_find_get_window(struct cmd_find_state *fs, const char *window,
                               int only) {
 ...
 * wl and w.
 */
static int cmd_find_get_window_with_session(struct cmd_find_state *fs,
                                            const char *window) {
  struct winlink *wl;
  const char *errstr;
  int idx, n, exact;
...
static int cmd_find_get_pane(struct cmd_find_state *fs, const char *pane,
                             int only) {
 ...
 * and w and wp.
 */
static int cmd_find_get_pane_with_session(struct cmd_find_state *fs,
                                          const char *pane) {
  log_debug("%s: %s", __func__, pane);
 ...
 * wp.
 */
static int cmd_find_get_pane_with_window(struct cmd_find_state *fs,
                                         const char *pane) {
  const char *errstr;
  int idx;
  struct window_pane *wp;
...
/* Log the result. */
static void cmd_find_log_state(const char *prefix, struct cmd_find_state *fs) {
  if (fs->s != NULL) {
...
void cmd_find_from_session(struct cmd_find_state *fs, struct session *s,
                           int flags) {
...
void cmd_find_from_winlink(struct cmd_find_state *fs, struct winlink *wl,
                           int flags) {
  cmd_find_clear_state(fs, flags);
...
int cmd_find_from_session_window(struct cmd_find_state *fs, struct session *s,
...
int cmd_find_from_window(struct cmd_find_state *fs, struct window *w,
                         int flags) {
  cmd_find_clear_state(fs, flags);
...
void cmd_find_from_winlink_pane(struct cmd_find_state *fs, struct winlink *wl,
...
int cmd_find_from_pane(struct cmd_find_state *fs, struct window_pane *wp,
                       int flags) {
...
int cmd_find_from_mouse(struct cmd_find_state *fs, struct mouse_event *m,
                        int flags) {
  cmd_find_clear_state(fs, flags);
...
int cmd_find_from_client(struct cmd_find_state *fs, struct client *c,
                         int flags) {
  struct window_pane *wp;
 ...
 */
int cmd_find_target(struct cmd_find_state *fs, struct cmdq_item *item,
  ...
  *tmp = '\0';
  ...
  } else if (cmd_find_from_client(&current, cmdq_get_client(item), flags) ==
             0) {
    fs->current = &current;
  ...
  if (session != NULL || window != NULL || pane != NULL) {
    log_debug("%s: target %s is %s%s%s%s%s%s", __func__, target,
              session == NULL ? "" : "session ", session == NULL ? "" : session,
              window == NULL ? "" : "window ", window == NULL ? "" : window,
              pane == NULL ? "" : "pane ", pane == NULL ? "" : pane);
  }
...
static struct client *cmd_find_current_client(struct cmdq_item *item,
                                              int quiet) {
...
struct client *cmd_find_client(struct cmdq_item *item, const char *target,
                               int quiet) {
  struct client *c;
  char *copy;
  size_t size;
