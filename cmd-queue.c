...
const char *cmdq_name(struct client *c);
/*
...
struct cmdq_list *cmdq_get(struct client *c);
/*
...
// const char *cmdq_get_name(struct cmdq_item *item) { return item->name; }

struct client *cmdq_get_client(struct cmdq_item *item);
// struct client *cmdq_get_client(struct cmdq_item *item) { return item->client; }

struct client *cmdq_get_target_client(struct cmdq_item *item);
//struct client *cmdq_get_target_client(struct cmdq_item *item) { return item->target_client; }

struct cmdq_state *cmdq_get_state(struct cmdq_item *item);
// struct cmdq_state *cmdq_get_state(struct cmdq_item *item) { return item->state; }
...
// struct cmd_find_state *cmdq_get_source(struct cmdq_item *item) { return &item->source; }

struct key_event *cmdq_get_event(struct cmdq_item *item);
// struct key_event *cmdq_get_event(struct cmdq_item *item) { return &item->state->event; }

struct cmd_find_state *cmdq_get_current(struct cmdq_item *item);
// struct cmd_find_state *cmdq_get_current(struct cmdq_item *item) { return &item->state->current; }

int cmdq_get_flags(struct cmdq_item *item);
// int cmdq_get_flags(struct cmdq_item *item) { return item->state->flags; }


struct cmdq_state *cmdq_new_state(struct cmd_find_state *current, struct key_event *event, int flags);
/*
struct cmdq_state *cmdq_new_state(struct cmd_find_state *current,
...
// struct cmdq_state *cmdq_link_state(struct cmdq_state *state) { state->references++; return state; }

struct cmdq_state *cmdq_copy_state(struct cmdq_state *state, struct cmd_find_state *current);
// struct cmdq_state *cmdq_copy_state(struct cmdq_state *state, struct cmd_find_state *current) { if (current != NULL) { return cmdq_new_state(current, &state->event, state->flags); } return cmdq_new_state(&state->current, &state->event, state->flags); }

void cmdq_free_state(struct cmdq_state *state);
// void cmdq_free_state(struct cmdq_state *state) { if (--state->references != 0) { return; } if (state->formats != NULL) { format_free(state->formats); } free(state); }
...
void cmdq_add_formats(struct cmdq_state *state, struct format_tree *ft);
/*
void cmdq_add_formats(struct cmdq_state *state, struct format_tree *ft) {
  if (state->formats == NULL) {
    state->formats = format_create(NULL, NULL, FORMAT_NONE, 0);
...
struct cmdq_item *cmdq_insert_after(struct cmdq_item *after,
                                    struct cmdq_item *item) {
  ...
  do {
    ...
    log_debug("%s %s: %s after %s", __func__, cmdq_name(c), item->name,
              after->name);
...
/*
void cmdq_insert_hook(struct session *s, struct cmdq_item *item,
...
// void cmdq_continue(struct cmdq_item *item) { item->flags &= ~CMDQ_WAITING; }

void cmdq_remove(struct cmdq_item *item);
/*
...
void cmdq_remove_group(struct cmdq_item *item);
/*
...
enum cmd_retval cmdq_empty_command(__unused struct cmdq_item *item, __unused void *data);
// static enum cmd_retval cmdq_empty_command(__unused struct cmdq_item *item, __unused void *data) { return CMD_RETURN_NORMAL; }
...
enum cmd_retval cmdq_find_flag(struct cmdq_item *item, struct cmd_find_state *fs, const struct cmd_entry_flag *flag);
/*
...
void cmdq_add_message(struct cmdq_item *item);
/*
static void cmdq_add_message(struct cmdq_item *item) {
  ...
  if (c != NULL) {
    ...
    if (c->session != NULL && state->event.key != KEYC_NONE) {
      ...
      server_add_message("%s%s key %s: %s", c->name, user, key, tmp);
    } else {
      server_add_message("%s%s command: %s", c->name, user, tmp);
    }
    free(user);
  } else {
...
enum cmd_retval cmdq_fire_command(struct cmdq_item *item);
/*
...
out:
  ...
  if (retval == CMD_RETURN_ERROR) {
    ...
    cmdq_insert_hook(fsp != NULL ? fsp->s : NULL, item, fsp, "command-error");
...
// struct cmdq_item *cmdq_get_error(const char *error) { return cmdq_get_callback(cmdq_error_callback, xstrdup(error)); }

enum cmd_retval cmdq_fire_callback(struct cmdq_item *item);
// static enum cmd_retval cmdq_fire_callback(struct cmdq_item *item) { return item->cb(item, item->data); }
...
u_int cmdq_next(struct client *c) {
  ...
  for (;;) {
    ...
    log_debug("%s %s: %s (%d), flags %x", __func__, name, item->name,
...
void cmdq_print_data(struct cmdq_item *item, int parse, struct evbuffer *evb);
// void cmdq_print_data(struct cmdq_item *item, int parse, struct evbuffer *evb) { server_client_print(item->client, parse, evb); }
