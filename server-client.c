...
static uint64_t server_client_key_table_activity_diff(struct client *c) {
  struct timeval diff;
...
static key_code server_client_check_mouse(struct client *c,
  ...
  if (event->key == KEYC_DOUBLECLICK) {
    ...
    log_debug("double-click at %u,%u", x, y);
             ...
             (m->sgr_type == ' ' && MOUSE_DRAG(m->b) && MOUSE_RELEASE(m->b) &&
              MOUSE_RELEASE(m->lb))) {
    type = MOVE;
    x = m->x, y = m->y, b = 0;
...
/* Resize timer event. */
static void server_client_resize_timer(__unused int fd, __unused short events,
                                       void *data) {
...
static void server_client_check_pane_resize(struct window_pane *wp) {
  ...
  TAILQ_FOREACH(r, &wp->resize_queue, entry) {
    log_debug("queued resize: %ux%u -> %ux%u", r->osx, r->osy, r->sx, r->sy);
  }
 ...
 */
static void server_client_check_modes(struct client *c) {
  struct window *w = c->session->curw->window;
  struct window_pane *wp;
  struct window_mode_entry *wme;
...
/* Set client path. */
static void server_client_set_path(struct client *c) {
  struct session *s = c->session;
  const char *path;
...
static enum cmd_retval server_client_read_only(struct cmdq_item *item,
                                               __unused void *data) {
...
static void server_client_dispatch_identify(struct client *c,
  ...
  case MSG_IDENTIFY_TERMINFO:
    ...
    c->term_caps =
        xreallocarray(c->term_caps, c->term_ncaps + 1, sizeof *c->term_caps);
...
static uint64_t server_client_control_flags(struct client *c,
                                            const char *next) {
...
/* Get client window. */
struct client_window *server_client_get_client_window(struct client *c,
                                                      u_int id) {
...
/* Add client window. */
struct client_window *server_client_add_client_window(struct client *c,
                                                      u_int id) {
...
/* Print to a client. */
void server_client_print(struct client *c, int parse, struct evbuffer *evb) {
